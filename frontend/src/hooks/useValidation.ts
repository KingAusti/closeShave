...
export const useValidation = () => {
  ...
  const validateQuery = useCallback(async (query: string): Promise<ValidationResponse> => {
    if (!query || !query.trim()) {
    ...
    try {
      ...
      const response = await axios.post<ValidationResponse>(`${API_BASE}/api/validate`, request, {
