...
class WalmartScraper(BaseScraper):
    ...
    def _parse_results(self, soup: BeautifulSoup, max_results: int) -> List[Product]:
        ...
        for container in containers:
            try:
                # Title
                title_elem = container.select_one(self.selectors.get("title", "[data-automation-id='product-title']"))
                ...
                # Image
                image_elem = container.select_one(self.selectors.get("image", "img[data-testid='product-image']"))
                image_url = self._extract_image_url(image_elem, "https://www.walmart.com")
                direct_image_url = image_url
                
                # Link
                link_elem = container.select_one(self.selectors.get("link", "a[data-testid='product-title']"))
                product_url = self._extract_url(link_elem, "https://www.walmart.com")
                
                # Availability
                availability_elem = container.select_one(self.selectors.get("availability", "[data-testid='product-availability']"))
